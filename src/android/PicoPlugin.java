...
public class PicoPlugin extends CordovaPlugin implements PicoConnectorListener, PicoListener {
    ...
    /**
     * scan raw data - uses the native sendRawDataRequest which fires each LED individually
     * and returns 9 raw sensor channel values (3 LEDs x 3 sensor channels)
     */
    ...
    @Override
    public void onFetchRawData(Pico pico, int[] data) {
        log("Received raw data: " + java.util.Arrays.toString(data));

        if (_curScanRawCallbackContext != null) {
            _curScanRawCallbackContext.success(java.util.Arrays.toString(data));
        }

        // broadcast raw scan data (9 values: 3 LEDs x 3 sensor channels)
        final Bundle rawScanBundle = new Bundle();
        rawScanBundle.putIntArray("raw", data);
